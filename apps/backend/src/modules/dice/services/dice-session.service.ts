...
const KEY = (id: string): string => `dice:session:${id}`;
const TTL = 60 * 60;

@Injectable()
export class DiceSessionService {
  constructor(
  ...
  async create(input: {
    ...
    clientSeed?: string;
    mode?: string;
  }): Promise<{ session: DiceSessionView; ruleset: DiceGameRuleSet }> {
    ...
    const record: DiceSessionRecord = {
  ...
  async get(id: string, userId: string): Promise<DiceSessionView> {
    return this.toView(await this.getRecord(id, userId));
  }

  /** Roll the dice and settle bets for the next nonce. */
  async roll(id: string, userId: string, bets: DiceBet[]): Promise<DiceRoundResult> {
    ...
    const result = engine.roll(`${record.sessionId}:${record.nonce}`, bets);
  ...
  async getRecord(id: string, userId: string): Promise<DiceSessionRecord> {
    const record = await this.redis.get<DiceSessionRecord>(KEY(id));
    if (!record) throw new NotFoundException('Dice session not found or expired');
    if (record.userId !== userId) throw new ForbiddenException('Not your dice session');
    return record;
  }

  private persist(record: DiceSessionRecord): Promise<void> {
    return this.redis.set(KEY(record.sessionId), record, TTL);
  }

  private requirePersisted(record: DiceSessionRecord): string {
    if (!record.gameSessionId) {
  ...
  private async persistResult(record: DiceSessionRecord, result: DiceRoundResult): Promise<void> {
    if (!record.gameSessionId) return;
    ...
    await this.wallet.settleImmediate({
      ...
      betAmount: result.settlement.totalBet,
      winAmount: result.settlement.totalWin,
      reference: `dice:${result.roundId}`,
      idempotencyKey: `dice:${result.roundId}`,
    });
  }

  private withVerification(
    result: DiceRoundResult,
    record: DiceSessionRecord,
    nonce: number,
    seed: string,
  ): DiceRoundResult {
    return {
  ...
  private toView(record: DiceSessionRecord): DiceSessionView {
    return {
